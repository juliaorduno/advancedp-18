...
{
    hash->size = size;
    hash->data = (HashElement *)calloc(size, size * sizeof(HashElement));
}
...
static void hashDoInsert(HashInt *hash, char *key, int value)
{
    ...
    if (currentHashElement->count == currentHashElement->length)
    {
        currentHashElement->length *= 2;
